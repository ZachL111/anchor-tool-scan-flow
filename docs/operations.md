...
`anchor-tool-scan-flow` is centered on cli tools work in Python. The technical target is: Package a Python local lab for scan analysis with deny and allow fixtures, explainable decision traces, and documented operating limits.
...
The score is deterministic. The current threshold is 172. Risk has a penalty of 5, latency has a penalty of 4, and weight contributes a bonus of 5.
